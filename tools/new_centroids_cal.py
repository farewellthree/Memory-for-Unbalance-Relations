...
def get_centroids():
    ...
    path_str = '/home/space/wwt/ECCV2020/Loutput2.0/e2e_faster_rcnn_VGG16_8_epochs_vg_v3_default_node_contrastive_loss_w_so_p_aware_margin_point2_so_weight_point5_no_spt/stage_one_1/ckpt/model_step62722.pth'
    ...
    with torch.set_grad_enabled(False):
        ...
        while(True):
            ...
            mix_cls_feat,mix_score,prd_label = get_feat(model,data,im_info,roidb_)
            pred = mix_score.max(dim=1)[1].cpu().numpy().astype(prd_label)
    ...
    mix_centroids /= torch.from_numpy(class_data_num).float().unsqueeze(1).cuda()
...
def get_feat(model,data,im_info,roidb,dataset_name='vg_train'):
    ...
    mix_score = model.RelDN.mix_scores(mix_cls_feat)
    #return prd_cls_feats,spt_cls_feats,prd_label
    return mix_cls_feat,mix_score,prd_label
