...
def save_ckpt(output_dir, args, step, train_size, model, optimizer):
    ...
    save_name_memory = os.path.join(ckpt_dir, 'memory_end{}.npy'.format(step))
    memory_end = model.module.RelDN.memory_save / model.module.RelDN.class_count.float().unsqueeze(1)
    np.save(save_name_memory,memory_end.cpu().numpy())
