...
class RelationLoss(nn.Module):
    def __init__(self, num_classes, feat_dim, size_average=True):
        ...
        self.RelationLossPos = RelationLossPos.apply
        self.feat_dim = feat_dim
        self.size_average = size_average

    def forward(self, feat, label, memory):
        ...
        loss_attract = self.RelationLossPos(feat, label, memory, batch_size_tensor).squeeze()
        
        # calculate repelling loss

        distmat = torch.pow(feat, 2).sum(dim=1, keepdim=True).expand(batch_size, self.num_classes) + \
                  torch.pow(memory, 2).sum(dim=1, keepdim=True).expand(self.num_classes, batch_size).t()
        distmat.addmm_(1, -2, feat, memory.t())
...
class RelationLossPos(Function):
    @staticmethod
...
def create_loss (feat_dim=512, num_classes=1000):
    ...
    return RelationLoss(num_classes, feat_dim)
