...
from modeling_rel.MetaEmbeddingClassifier import MetaEmbedding_Classifier
from modeling_rel.RelationLoss import RelationLoss
...
class reldn_head(nn.Module):
    def __init__(self, dim_in):
        ...
        self.mix_feats = nn.Sequential(
            nn.Linear(dim_in_final+64,1024),
            nn.LeakyReLU(0.1))
        self.mix_scores = MetaEmbedding_Classifier(1024,num_prd_classes)  

        self.memory_train = Variable(torch.zeros(num_prd_classes, 1024),requires_grad=False).cuda()
        self.memory_save = torch.zeros(num_prd_classes, 1024).cuda()
        self.class_count = torch.zeros(num_prd_classes).cuda()
        
        self.mix_cent_loss = RelationLoss(num_prd_classes,1024)
        self._init_weights() 
    ...
    # spt_feat是空间分数计算的输入
    # spo_feat是视觉分数计算的输入
    def forward(self, spo_feat, spt_feat=None, sbj_labels=None, obj_labels=None, sbj_feat=None, obj_feat=None, rel_ret=None):
        device_id = spo_feat.get_device()
        if self.training:
            label = rel_ret['all_prd_labels_int32']
            labels = Variable(torch.from_numpy(label.astype('int64'))).cuda(device_id)
        ...
        mix_cls_feat = self.mix_feats(mix_feat)

        if self.training and cfg.MODEL.MEMORY_TRAIN_UPDATE:
            for i in range(len(label)):
                single_label = label[i]
                self.memory_train.data[single_label] = ( self.memory_train.data[single_label] + \
                    0.01 * mix_cls_feat [i]) / (1 + 0.01)   
                    
            mix_feat_loss = self.mix_cent_loss(mix_cls_feat,labels,self.memory_train)*0.01

        mix_score, veta_feat = self.mix_scores(mix_cls_feat,self.memory_train)  

        if self.training and cfg.MODEL.MEMORY_SAVE_UPDATE: 
            logits = F.softmax(mix_score, dim=1)
            for i in range(len(label)):
                single_label = label[i]
                single_score = logits.data[i][single_label]
                self.memory_save[single_label] += single_score * mix_cls_feat[i].data
                self.class_count[single_label] += single_score
        ...
        if cfg.MODEL.ADD_SCORES_ALL:
            ttl_cls_scores = mix_score
            if cfg.MODEL.USE_FREQ_BIAS:
                ttl_cls_scores += prd_bias_scores #语义分数！！！！
            # if cfg.MODEL.USE_SPATIAL_FEAT:
            #     ttl_cls_scores += prd_spt_scores  #空间分数！！！！
            # if cfg.MODEL.ADD_SO_SCORES:
        ...
        return prd_vis_scores, prd_bias_scores, prd_spt_scores, ttl_cls_scores, sbj_cls_scores, obj_cls_scores, mix_feat_loss
