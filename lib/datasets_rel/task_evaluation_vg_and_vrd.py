...
def eval_rel_results(all_results, output_dir, do_val):
    ...
    eval_sets = (False,)
    print (cfg.MODEL.NUM_PRD_CLASSES)
    for phrdet in eval_sets:
        ...
        for prd_k in prd_k_set:
            ...
            label_prd = np.zeros(cfg.MODEL.NUM_PRD_CLASSES)
            recall_prd = {20: np.zeros(cfg.MODEL.NUM_PRD_CLASSES), 50: np.zeros(cfg.MODEL.NUM_PRD_CLASSES), 100: np.zeros(cfg.MODEL.NUM_PRD_CLASSES)}
            ...
            for im_i, res in enumerate(tqdm(all_results)):
                ...
                if False:#res['prd_scores'] is None:
                ...
                if do_val:
                    ...
                    for k in recalls:
                        ...
                        for match_ind in match:
                            prd_ind = gt_labels_prd[int(match_ind)]
                            recall_prd[k][prd_ind] += 1
            ...
            if do_val:
                for k in recalls:
                    ...
                    for i in range(cfg.MODEL.NUM_PRD_CLASSES):
                        recall_prd[k][i] /= label_prd[i]

                for k, v in recalls.items():
                    print('R@%i: %.2f' % (k, 100 * v))
                for k, v in recall_prd.items():
                    print('mR@%i: %.2f' % (k, 100 * np.mean(v)))
            recalls_all[prd_k] = recalls
            recall_prd_all[prd_k] = recall_prd
        
        for k,v in recalls_all.items():
            print(k,':')
            for kk,vv in v.items():
                print('R@%i: %.2f' % (kk, 100 * vv))
        
        for k,v in recall_prd_all.items():
            print(k,':')
            for kk,vv in v.items():
                print('mR@%i: %.2f' % (kk, 100 * np.mean(vv)))
